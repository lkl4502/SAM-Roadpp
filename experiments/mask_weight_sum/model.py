...
class SAMRoadplus(pl.LightningModule):
    ...
    def forward(self, rgb, keypoint_mask, road_mask):
        ...
        if self.config.USE_SAM_DECODER:
            ...
            if self.config.ALEATORIC:
                ...
                mask_scores = torch.cat(
                    [keypoint_score, road_score], dim=1
                )  # B, 2, H, W
            else:
                mask_scores = torch.sigmoid(mask_logits)

        # Gt, Prediction Mask Weight Sum
        gt_mask = torch.stack([keypoint_mask, road_mask], dim=1)  # B, 2, H, W
        mask_scores = (
            self.config.MASK_WEIGHT * mask_scores
            + (1 - self.config.MASK_WEIGHT) * gt_mask
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        (
            ...
            _,  # topo_scores,
            valid,
        ) = self(rgb, keypoint_mask, road_mask)
