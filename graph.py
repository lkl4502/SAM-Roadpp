...
class RoadGraph:
            ...
            for t in forest:
                for n in t:
                    ...
                    if n["similarWith"][0] != -1:
                        ...
                        thislat = forest[n["similarWith"][0]][n["similarWith"][1]][
                            "lat"
                        ]
                        thislon = forest[n["similarWith"][0]][n["similarWith"][1]][
                            "lon"
                        ]
                    ...
                    self.addEdge(
                        idparent, parentlat, parentlon, idthis, thislat, thislon
                    )
    ...
    def BiDirection(self):
        ...
        for edge in edgeList:
            ...
            self.edges[self.edgeID] = [localid1, localid2]
            self.edgeHash[localid1 * 10000000 + localid2] = self.edgeID
            self.edgeScore[self.edgeID] = self.edgeScore[
                self.edgeHash[localid2 * 10000000 + localid1]
            ]
            self.edgeID += 1
        ...
        def explore(node_cur, node_prev, dist):
            ...
            for next_node in self.nodeLink[node_cur] + reverseList:
                ...
                if node_cur * 10000000 + next_node in self.edgeHash.keys():
                    edgeS = self.edgeScore[
                        self.edgeHash[node_cur * 10000000 + next_node]
                    ]

                if next_node * 10000000 + node_cur in self.edgeHash.keys():
                    edgeS = max(
                        edgeS,
                        self.edgeScore[self.edgeHash[next_node * 10000000 + node_cur]],
                    )
                ...
                if old_node_dist + l < r:
                    ...
                    while cur < l:
                        ...
                        if (
                            ...
                            lat2 - lat1,
                            lon2 - lon1,
                        ) not in mables and turnnel_edge is False:
                            mables.append(
                                (latI, lonI, lat2 - lat1, lon2 - lon1)
                            )  # add direction
                            ...
                            if bidirection == True:
                                if (
                                    next_node in self.nodeLink[node_cur]
                                    and node_cur in self.nodeLink[next_node]
                                    and turnnel_edge is False
                                ):
                                    mables.append(
                                        (
                                            latI + 0.00001,
                    ...
                    if (node_cur, next_node) in edge_covered.keys():
                        ...
                        edge_covered[(node_cur, next_node)] = (
                            cur - step
                        )  # max(cur, edge_covered[(node_cur, next_node)])
    ...
    def removeDeadEnds(self, oneround=False):
        ...
        for nodeid in self.nodes.keys():
            ...
            if (
                d == 1
                or len(self.nodeLink[nodeid]) == 0
                or len(self.nodeLinkReverse[nodeid]) == 0
            ):
