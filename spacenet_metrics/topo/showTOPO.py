...
def RenderSVG(
    ...
    if OSMMap is not None:
        ...
        for edgeId, edge in roadmap.edges.iteritems():
            ...
            ilat2, ilon2 = Coord2Pixels(
                lat1, lon1, min_lat, min_lon, max_lat, max_lon, sizex, sizey
            )
            ilat, ilon = Coord2Pixels(
                lat2, lon2, min_lat, min_lon, max_lat, max_lon, sizex, sizey
            )
    ...
    if starts is not None:
        x, y = Coord2Pixels(
            starts[0], starts[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey
        )
        dwg.add(dwg.circle(center=(y, x), r=3, stroke="orange", fill="none"))

        x, y = Coord2Pixels(
            starts[2], starts[3], min_lat, min_lon, max_lat, max_lon, sizex, sizey
        )
    ...
    for hole in holes:
        ...
        x, y = Coord2Pixels(
            hole[0], hole[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey
        )
    ...
    for marble in marbles:
        ...
        x, y = Coord2Pixels(
            marble[0], marble[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey
        )
