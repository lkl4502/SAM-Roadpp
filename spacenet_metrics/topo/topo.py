...
def TOPOGenerateStartingPoints(
    OSMMap,
    check=True,
    density=0.00050,
    region=None,
    ...
    for nodeid in OSMMap.nodes.keys():
        ...
        for nextnode in next_nodes.keys():
            ...
            for i in range(0, len(node_list) - 1):
                dists.append(dist)
                dist += distance(
                    OSMMap.nodes[node_list[i]], OSMMap.nodes[node_list[i + 1]]
                )
            ...
            for alpha in alphas:
                for j in range(len(node_list) - 1):
                    ...
                    if metaData is not None:
                        ...
                        if (
                            metaData.edgeProperty[metaData.edge2edgeid[(nnn1, nnn2)]][
                                "layer"
                            ]
                            < 0
                        ):
                    ...
                    if alpha * dist >= dists[j] and alpha * dist <= dists[j + 1]:
                        ...
                        lat = (1 - a) * OSMMap.nodes[node_list[j]][
                            0
                        ] + a * OSMMap.nodes[node_list[j + 1]][0]
                        lon = (1 - a) * OSMMap.nodes[node_list[j]][
                            1
                        ...
                        if img != None:
                            ...
                            if (
                                lat - region[0] > lat_mergin
                                and region[2] - lat > lat_mergin
                                and lon - region[1] > lon_mergin
                                and region[3] - lon > lon_mergin
                            ):
                                result.append(
                                    (
                                        lat,
    ...
    for _, edge in OSMMap.edges.items():
        ...
        svgEdges.append(
            (
                OSMMap.nodes[edge[0]][0],
                OSMMap.nodes[edge[0]][1],
                OSMMap.nodes[edge[1]][0],
                OSMMap.nodes[edge[1]][1],
            )
        )
...
def TOPOGenerateList(
    ...
    for idthis in OSMMap.nodes.keys():
        x, y = Coord2Pixels(
            OSMMap.nodes[idthis][0],
            OSMMap.nodes[idthis][1],
            region[0],
            region[1],
            region[2],
            region[3],
            ...
            if img[x, y] > 0:
                idx.insert(
                    ...
                    (
                        OSMMap.nodes[idthis][0],
                        OSMMap.nodes[idthis][1],
                        OSMMap.nodes[idthis][0] + 0.000001,
                        OSMMap.nodes[idthis][1] + 0.000001,
                    ),
                )
    ...
    for nid in candidateNode.keys():
        ...
        possible_nodes = list(
            idx.intersection(
                (lat - threshold, lon - threshold, lat + threshold, lon + threshold)
            )
        )
...
def TOPO121(topo_result, roadgraph):
    ...
    for ind in range(len(topo_result)):
        ...
        candidate = list(
            rtree_index.intersection(
                [lat - r_lat, lon - r_lon, lat + r_lat, lon + r_lon]
            )
        )
        ...
        for can_id in candidate:
            t_gpsn1, t_gpsn2, t_gpsd1, t_gpsd2 = (
                topo_result[can_id][4],
                topo_result[can_id][5],
                topo_result[can_id][6],
                topo_result[can_id][7],
            )
        ...
        for marble in marbles:
            ...
            possible_holes = list(
                idx_holes_bidirection.intersection(
                    (marble[0] - rr, marble[1] - rr, marble[0] + rr, marble[1] + rr)
                )
            )
            for hole_id in possible_holes:
                ...
                if (
                    ddd < threshold and angle_d < 0.29
                ):  # 0.03 --> 15 degrees  0.13 --> 30 degrees 0.29 --> 45 degrees
...
def TOPOWithPairsNew(
    ...
    for k, itemGPS in GPSList.items():
        ...
        showTOPO.RenderSVG(
            ...
            lon,
            0.00500,
            "svg/" + svgname + str(i) + "_" + str(lat) + "_" + str(lon) + ".svg",
        )
        ...
        if len(marbles) == 0:
            ...
            smooth_precision = 1.0 - (
                cost + (len(marbles) - matchedNum) * threshold * 1.15
            ) / (len(marbles) * threshold * 1.15)

            smooth_recall = 1.0 - (cost + (base_n - matchedNum) * threshold * 1.15) / (
                base_n * threshold * 1.15
            )
            ...
            if smooth_recall + smooth_precision == 0:
                ...
                f = (
                    ...
                    * smooth_precision
                    * smooth_recall
                    / (smooth_recall + smooth_precision)
                )
    ...
    return (
        ...
        total_f,
        number_of_holes,
    )  # cost+ (len(marbles) - matchedNum) * threshold * 3
...
def TOPOWithList(
    ...
    for min_node in pairs.keys():
        ...
        showTOPO.RenderSVG(
            ...
            lon,
            0.00500,
            "svg/" + str(i) + "_" + str(lat) + "_" + str(lon) + ".svg",
        )
