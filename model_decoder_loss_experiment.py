        ...
        if self.config.COMBINE_LOSS:  # True면 우선 KL Divergence loss사용
            ...
            for i in range(self.config.DECODER_COUNT):  # Decoder 순으로 pair 구성
                for j in range(i + 1, self.config.DECODER_COUNT):
                    kl_div_loss = self.kl_div_criterion(
                        mask_scores_list[i].log(), mask_scores_list[j]
                    )
                    self.log(
                        f"train_kl_div_loss_{i}_{j}",
                        kl_div_loss,
                        on_step=True,
                        on_epoch=False,
                        prog_bar=True,
                    )
                    kl_div_loss_list.append(kl_div_loss)

            total_kl_div_loss = torch.stack(kl_div_loss_list).mean()
            self.log(
                "train_total_kl_div_loss",
                total_kl_div_loss,
                on_step=True,
                on_epoch=False,
                prog_bar=True,
            ...
            total_loss -= self.config.KL_DIV_LOSS_WEIGHT * total_kl_div_loss
